...
#----------------------------------------------------------------------------------------------------
 ...
 def init_evb(self, channels=[0]):
  ...
  for ch in channels:
   lane_en += 1<<ch
  self.SetConfig('lane_en',lane_en)
  self.mApb.init_evb()
  self.mPmad.Init()
 ...
 def meas_ber (self,measure_bits_db=30 ,channel=0):
  return self.mPmad.GetBer(measure_bits_db,channel)

 def get_status (self,measure_bits_db=30,HeightOnly=0,extra_ber_en=1,lin_fit_en=1,lin_fit_point=41,lin_fit_main=10,imp_eq_out=0,channel=0):
 ...
 def GetHistogram(self,channel=0):
  self.mCs.get_histogram(channel)
