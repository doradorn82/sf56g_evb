...
#{{{
	...
	def GetStatus(self,measure_bits_db=30,extra_ber_en=1,HeightOnly=1,lin_fit_en=1,lin_fit_point=41,lin_fit_main=10,imp_eq_out=0,channel=0):
		...
		if(result['ber']==0 and extra_ber_en==1):
			if(self.is_pam4_rate(self.mCfg.data_rate)):
				result['ber']=self.get_extra_ber(nBit=2,ln_i=channel)
			else:
				result['ber']=self.get_extra_ber(nBit=1,ln_i=channel)
	...
	def get_extra_ber(self, nBit, ln_i):
		...
		for mem_i in range(memSize):
			...
			fs.write("%s %s\n" % (str(mem_i), str(data)))
		fs.close()
		self.mApb.write(0x00061000, 0x5<<3, channel=ln_i, mask=0xf<<3)
		if self.is_bbpd_rate(self.mCfg.data_rate):
			countnum = 17
		else:
			countnum = 19
		self.mApb.write(0x00061000, countnumOrg<<3, channel=ln_i, mask=0xf<<3)
		filename2 = self.mCfg.dump_abs_path + "histo_data_" + "ln" + str(ln_i) + "_count" + str(countnum) + ".txt"
		dataTemp = self.mApb.read(0x00060100, ln_i)
		dataTemp = dataTemp & 0xBF
		dataTemp = dataTemp | 0x40
		self.mApb.write(0x00060100, dataTemp & 0x7F, ln_i)
		...
		if (nBit == 2):
			return evb_extra.bathtub_extrapolation_pam4(filename, filename2, 29, 19)
		else:
			return evb_extra.bathtub_extrapolation_nrz(filename, filename2, 29, 19,vrefSel)
